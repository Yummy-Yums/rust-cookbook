...
fn main() {
    ...
    let date_regex_using_capture_groups =
        Regex::new(r"(\d{2}).(\d{2}).(\d{4})")
            .expect("Failed to create regex");

    let text_with_dates =
        "Alan Turing was born on 23.06.1912 and
         died on 07.06.1954. A movie about his life
         called 'The Imitation Game' came out on 14.11.2017";

    for cap in date_regex_using_capture_groups.captures_iter(text_with_dates) {
    ...
    let text_with_american_dates =
        date_regex.replace_all(text_with_dates, "$month/$day/$year");
